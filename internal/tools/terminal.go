...
func RunTerminal(ctx context.Context, workDir, command string) (stdout, stderr string, exitCode int, err error) {
	var shell string
	var args []string
...
func RunTerminalWithTimeout(ctx context.Context, workDir, command string, timeout time.Duration) (stdout, stderr string, exitCode int, err error) {
	if timeout <= 0 {
		timeout = 5 * time.Minute
	}
	ctx, cancel := context.WithTimeout(ctx, timeout)
	defer cancel()
	return RunTerminal(ctx, workDir, command)
}
...
func RunTerminalTool(ctx context.Context, workDirDefault string, argsJSON string) (string, error) {
	...
	stdout, stderr, code, _ := RunTerminalWithTimeout(ctx, args.WorkDir, args.Command, 5*time.Minute)
